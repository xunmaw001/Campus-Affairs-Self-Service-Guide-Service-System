...
public class ZhinanLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody ZhinanLiuyanEntity zhinanLiuyan, HttpServletRequest request){
...
//            zhinanLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ZhinanLiuyanEntity> queryWrapper = new EntityWrapper<ZhinanLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ZhinanLiuyanEntity zhinanLiuyanEntity = zhinanLiuyanService.selectOne(queryWrapper);
        zhinanLiuyan.setUpdateTime(new Date());
        if(zhinanLiuyanEntity==null){
            //  String role = String.valueOf(request.getSession().getAttribute("role"));
            //  if("".equals(role)){
            //      zhinanLiuyan.set
            //  }
...
//                            zhinanLiuyanEntity.setZhinanId(Integer.valueOf(data.get(0)));   //指南信息 要改的
...
//                            zhinanLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
